...
bitcoin-cli -regtest createwallet "btrustwallet" || bitcoin-cli -regtest loadwallet "btrustwallet" || true
...
# WRITE YOUR SOLUTION BELOW:
bitcoin-cli -regtest createwallet "treasurewallet" || bitcoin-cli -regtest loadwallet "treasurewallet" || true
...
# WRITE YOUR SOLUTION BELOW:
bitcoin-cli -regtest loadwallet "btrustwallet" || true
...
# WRITE YOUR SOLUTION BELOW:
LEGACY_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress 'legacy_address' 'legacy' )
check_cmd "Legacy address generation"

P2SH_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress 'p2sh_address' 'p2sh-segwit' )
check_cmd "P2SH address generation"

SEGWIT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress 'bech32_address' 'bech32' )
check_cmd "SegWit address generation"

TAPROOT_ADDR=$(bitcoin-cli -regtest -rpcwallet=btrustwallet getnewaddress 'bech32m_address' 'bech32m' )
...
# WRITE YOUR SOLUTION BELOW:
INTERNAL_KEY=$(echo "$ADDR_INFO" | jq -r '.desc' | sed -n 's/.*(\[.*\]\(.*\))#.*/\1/p')
